...
/**
 * Changes the remark of an existing record in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the record identified "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the record in the filtered record list to edit the remark
     * @param remark of the record to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of record data. We achieve that by working with the `Record` model. Each field in a Record is implemented as a separate class (e.g. a `Name` object represents the record’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a record.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.record`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each record.

Simply add the following to [`seedu.address.ui.RecordCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
Then insert the following into [`main/resources/view/RecordListCard.fxml`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-d44c4f51c24f6253c277a2bb9bc440b8064d9c15ad7cb7ceda280bca032efce9).
...
## Modify `Record` to support a `Remark` field

Since `RecordCard` displays data from a `Record`, we need to update `Record` to get our `Remark` displayed!

### Modify `Record`

We change the constructor of `Record` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `Record`

Unfortunately, a change to `Record` will cause other commands to break, you will have to modify these commands to use the updated `Record`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `Record` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedRecord` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedRecord` to work with our new `Record`!
...
Now that we have finalized the `Record` class and its dependencies, we can now bind the `Remark` field to the UI.
...
public RecordCard(Record record, int displayedIndex) {
    //...
    remark.setText(record.getRemark().value);
}
```
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Record`. Since all fields in a `Record` are immutable, we create a new instance of a `Record` with the values that we want and
save it with `Model#setRecord()`.

**`RemarkCommand.java`:**

``` java
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added remark to Record: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed remark from Record: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
        ...
        Record editedRecord = new Record(
                recordToEdit.getName(), recordToEdit.getPhone(), recordToEdit.getEmail(),
                recordToEdit.getAddress(), remark, recordToEdit.getTags());

        model.setRecord(recordToEdit, editedRecord);
        model.updateFilteredRecordList(PREDICATE_SHOW_ALL_RECORDS);

        return new CommandResult(generateSuccessMessage(editedRecord));
    }
     ...
     * {@code recordToEdit}.
     */
    private String generateSuccessMessage(Record recordToEdit) {
