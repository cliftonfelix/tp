...
1. The rest of the method seems to exhaustively check for the existence of each possible parameter of the `edit` command and store any possible changes in an `EditRecordDescriptor`. Recall that we can verify the contents of `editRecordDesciptor` through the 'Variables' window.<br>
...
1. As suspected, `command#execute()` does indeed make changes to the `model` object. Specifically,
   * it uses the `setRecord()` method (defined in the interface `Model` and implemented in `ModelManager` as per the usual pattern) to update the record data.
   * it uses the `updateFilteredRecordList` method to ask the `Model` to populate the 'filtered list' with _all_ records.<br>
     FYI, The 'filtered list' is the list of records resulting from the most recent operation that will be shown to the user immediately after. For the `edit` command, we populate it with all the records so that the user can see the edited record along with all other records. If this was a `find` command, we would be setting that list to contain the search results instead.<br>
     To provide some context, given below is the class diagram of the `Model` component. See if you can figure out where the 'filtered list' of records is being tracked.
...
1. It appears that a `JsonAdaptedRecord` is created for each `Record` and then added to the `JsonSerializableAddressBook`.
